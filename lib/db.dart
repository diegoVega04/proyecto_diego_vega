...
class Db {
  ...
  static Future<List<LibroUsuario>> getLibrosUsuario (Usuario user) async {
    ...
    final List<Map<String, dynamic>> librosMap = await database.query("libros_usuario", where: "id = ?", whereArgs: [user.id]);
    return List.generate(librosMap.length, 
      (i) => LibroUsuario(
        id_libro: librosMap[i]['idLibro'],
        id_usuario: librosMap[i]['idUsuario'],
